...
DEPLOY_VERSION="1.5prod_20220226"
VERSION=`cat VERSION`
...
echodo gcloud run deploy twitter-scraper \
     ...
     --set-env-vars="^###^MESSAGGIO_OCCASIONALE=[PROD] $MESSAGGIO_OCCASIONALE,DEPLOY_VERSION=$DEPLOY_VERSION" \
     ...
     --set-env-vars="APP_LOCATION=GCP CloudRun on $PROJECT_ID (prod)" \
     --platform managed \
     --region europe-west1 \
