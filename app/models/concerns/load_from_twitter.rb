...
module LoadFromTwitter
    ...
    def invoke_seeding_from_concern(search_key, search_count, opts={})
        ...
        twittersecret_api_key        = opts.fetch :twittersecret_api_key, ENV['TWITTER_CONSUMER_KEY'].remove_external_doublequotes # trim('"') 
...
def self.process_tweet_from_api_or_file(tweet, opts={}) 
  puts :TODOwrapper
end 

# TODO importantissimo: quando posso implementare questa in maniera pulita ora funge SIA da API che da file :)
def process_tweet_from_api_or_file(bool_source, tweet, opts={})
  # Nota che il tweet e' il Twitter tweet object, non un oggetto carlessiano.
  check_already_exists = opts.fetch :check_already_exists, false 
  ...
  search_term          = opts.fetch :api_search_term, "API only, :file doesnt know"
  search_count         = opts.fetch :api_search_count, 0 # same

  raise Exception("I only accept bool_source for value: :api and :file and :api") unless bool_source.in? [ :file, :api ]

  ret = {
    :n_unsaved_tweets => 0,
    :n_called_tweets => 1, # inutile, sara sempre uno
    :n_saved_users => 0,
    :n_saved_tweets => 0,
    # single error here.
  }
  #puts "DEB: twewet verbosissimodi Twitter: #{tweet}"
  ...
  # Tweet matches :)
  manage_twitter_serialization(tweet, opts) if marshal_on_file and bool_source == :api # otherwise we risk a deadlock
  ...
  if not wordle_type.nil?
    ...
    if check_already_exists
      ...
      return {:error => :todo_manage_this } if already_exists
    end
